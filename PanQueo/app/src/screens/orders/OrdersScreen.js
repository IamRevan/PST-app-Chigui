...
const STATUS_MAP = {
  Pendiente: {
    ...
    text: colors.pendiente,
    dot: "#FF9800",
  },
  Abonado: {
    border: "#2196F3",
    bg: colors.enCocinaBg,
    text: colors.enCocina,
    dot: "#2196F3",
  },
...
export const OrdersScreen = ({ navigation }) => {
  ...
  const { data: orders, loading, refetch } = usePedidos();

  const filtered = orders.filter((p) => {
    if (activeFilter === "Todos") return true;
    return p.estado_entrega === FILTER_MAP[activeFilter];
  });
    ...
    <ScreenWrapper>
      ...
      >
        ...
        {!loading &&
          filtered.map((order) => {
            const sc =
              STATUS_MAP[order.estado_entrega] || STATUS_MAP["Pendiente"];
            const estadoBadge =
              order.estado_entrega === "Por Preparar"
                ? "Pendiente"
                : order.estado_entrega;
            return (
              <View
                key={order.id_pedido}
                style={[styles.orderCard, { borderLeftColor: sc.border }]}
              >
                <View style={styles.orderTop}>
                  <View style={styles.orderTopLeft}>
                    ...
                    <Badge
                      label={estadoBadge}
                      variant={estadoBadge.toLowerCase().replace(/\s+/g, "-")}
                      dot
                    />
                    {order.es_delivery && (
                  ...
                  <Text style={styles.orderTotal}>
                    ${parseFloat(order.costo_total || 0).toFixed(2)}
                  </Text>
                </View>
                <View style={styles.orderDetails}>
                  <View style={styles.detailRow}>
                    ...
                    <Text style={styles.detailValue}>
                      {order.Cliente?.nombre || `ID ${order.id_cliente}`}
                    </Text>
                  </View>
                  <View style={styles.detailRow}>
        ...
        {!loading && filtered.length === 0 && (
          <Text style={styles.emptyText}>
            No hay pedidos{" "}
            {activeFilter !== "Todos" ? activeFilter.toLowerCase() : ""}
          </Text>
        )}
      </ScrollView>
    </ScreenWrapper>
